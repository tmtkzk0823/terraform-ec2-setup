...
resource "aws_security_group" "app_sg" {
  ...
  description = "application sever role security group"
  vpc_id      = aws_vpc.vpc.id
...
resource "aws_security_group_rule" "app_out_http" {
  ...
  from_port         = 80
  to_port           = 80
  prefix_list_ids   = [data.aws_prefix_list.s3_pr.id]
}
...
# operation server security group
resource "aws_security_group" "opmng_sg" {
  name        = "${var.project}-${var.environment}-opmng-sg"
  description = "operation management server role security group"
  vpc_id      = aws_vpc.vpc.id
